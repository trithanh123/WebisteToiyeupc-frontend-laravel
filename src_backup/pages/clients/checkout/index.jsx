  ...
  const handleCheckout = () => {
    ...
    const finishCheckout = async () => {
      try {
        ...
        const payload = {
          MaChiNhanh: selectedBranch ? (selectedBranch.id || selectedBranch.iD_ChiNhanh) : 1, 
          MaKhuyenMai: orderVoucher ? (orderVoucher.id_khuyenmai || orderVoucher.ID_KhuyenMai || orderVoucher.id) : null,
          MaDiachinguoidung: selectedHomeAddressId, 
          addressData: {
          ...
          cart_items: selectedProducts.map(item => ({
            MaSanPham: item.id || item.ID_SanPham,
        ...
        if (res.data.status === 'success') {
          ...
          // NẾU CÓ URL VNPAY THÌ CHUYỂN HƯỚNG SANG SANDBOX
    ...
    if (wantInvoice && invoiceEmail) {
      ...
      const templateParams = {
        ...
        orders: selectedProducts.map(item => ({
          ...
          image_url: 'https://placehold.co/100x100?text=SP', // Dùng ảnh mẫu vì ảnh local không hiển thị trong email
          units: item.quantity,
