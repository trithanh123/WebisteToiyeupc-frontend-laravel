...
const ProductCard = ({ product }) => {
  ...
  // Giả lập giá gốc (thực tế lấy từ DB nếu có trường gia_goc)
...
const ProductPage = () => {
  ...
  const fetchProducts = useCallback(async (page, sortKey, minP, maxP, filters = {}) => {
    ...
    try {
      ...
      // Append custom filters for backend to catch
  ...
  // ── Sinh dữ liệu Bộ Lọc động từ API Danh Mục ──
        ...
        <div className="product-page__inner">
          ...
          <aside className="product-sidebar" id="product-sidebar">
            ...
            <div className="product-sidebar__box">
              ...
              {/* Accordion Bộ Lọc Tự Động */}
              {dynamicFilterGroups.map(group => {
                const isOpen = expandedGroups[group.id] !== false; // Mặc định mở
