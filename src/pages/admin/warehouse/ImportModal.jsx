      ...
      const fetchData = async () => {
        try {
          ...
          const [branchRes, productRes] = await Promise.all([
            axios.get('https://webistetoiyeupc-backend-laravel.onrender.com/api/admin/branches', { headers }),
            axios.get('https://webistetoiyeupc-backend-laravel.onrender.com/api/admin/products', { headers })
          ]);
    ...
    try {
      ...
      if (san_phams.length === 0) {
        Swal.fire({
          ...
          text: 'Vui lòng chọn sản phẩm và nhập số lượng hoặc Serial hợp lệ!'
        });
      ...
      const response = await fetch('https://webistetoiyeupc-backend-laravel.onrender.com/api/admin/warehouse/store', {
        method: 'POST',
        headers: {
      ...
      if (response.ok && result.status === 'success') {
        ...
        let errorMessage = result.message || "Lỗi lưu phiếu nhập";
        if (result.errors) {
