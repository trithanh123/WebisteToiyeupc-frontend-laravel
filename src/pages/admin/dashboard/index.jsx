...
const API_BASE = "https://webistetoiyeupc-backend-laravel.onrender.com/api";

/**
 * DashboardContent – chỉ mount bên trong {children} của AdminMasterLayout,
 * tức là SAU KHI authState === "ok" và token đã có trong localStorage.
 * Không còn race condition với quá trình xác thực.
 */
const DashboardContent = () => {
  ...
  const fetchDashboardData = async (retryCount = 0) => {
    ...
    try {
      ...
      if (!token) {
        // Không nên xảy ra vì component này chỉ mount sau auth, nhưng phòng hờ:
        if (retryCount < 3) {
      ...
      const response = await fetch(
        `${API_BASE}/admin/dashboard?month=${month}&year=${year}`,
        {
          headers: {
      ...
      if (response.ok && result.status === "success") {
        setData(result.data);
      } else if (response.status === 401 && retryCount < 4) {
        // Backend cold start chưa kịp nhận diện token → thử lại sau 1.5s
  ...
  // useEffect này chỉ chạy khi DashboardContent mount –
  // tức là khi AdminMasterLayout đã xác thực xong (authState === "ok")
  useEffect(() => {
    fetchDashboardData();
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Tổng quan tình hình kinh doanh và kho hàng
          </p>
        </div>
...
/**
 * AdminDashboard – route component đơn giản, chỉ wrap AdminMasterLayout.
 * Không có state hay useEffect ở đây → không bao giờ fetch trước khi auth xong.
 */
