  ...
  return (
    ...
    >
      ...
      <div className="gvn-card__img-wrap">
        ...
        <button 
          onClick={(e) => { e.stopPropagation(); toggleWishlist(product.id_sanpham); }}
          className="absolute top-2 left-2 p-1.5 bg-white/80 rounded-full shadow-sm hover:scale-110 transition z-10">
          <svg className={`w-5 h-5 ${isInWishlist(product.id_sanpham) ? 'text-red-500 fill-current' : 'text-gray-400 fill-none'} transition-colors`} viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
