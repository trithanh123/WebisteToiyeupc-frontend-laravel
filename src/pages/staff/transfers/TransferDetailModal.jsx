...
const TransferDetailModal = ({ id, myBranchId, onClose, onUpdated }) => {
  ...
  const fetchDetail = async () => {
    ...
    try {
      ...
      if (res.ok && result.status === 'success') {
        ...
        // If status is "Chờ duyệt" AND I am the sender, fetch available serials
        if (data.trang_thai === 'Chờ duyệt' && myBranchId === data.ma_kho_xuat) {
          const serialsMap = {};
          const selMap = {};
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const result = await res.json();
      if (res.ok && result.status === 'success') {
        alert("Đã duyệt phiếu thành công. Hàng đang được xuất kho.");
        onUpdated();
      } else {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      const result = await res.json();
      if (res.ok && result.status === 'success') {
        alert("Đã nhận hàng thành công. Tồn kho đã được cập nhật vào chi nhánh của bạn.");
        onUpdated();
      } else {
        alert("Lỗi: " + result.message);
      }
  ...
  const isMyExport = ticket.ma_kho_xuat === myBranchId;
  const isMyImport = ticket.ma_kho_nhap === myBranchId;
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-[800px] max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-slate-50">
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className={`p-4 rounded-lg border ${isMyExport ? 'border-orange-400 bg-orange-50' : 'border-slate-200 bg-white'}`}>
              <div className="text-[11px] font-bold text-slate-400 uppercase mb-1">Kho Xuất (Nguồn) {isMyExport && <span className="text-orange-600 font-bold ml-1">(BẠN LÀ BÊN GỬI)</span>}</div>
              <div className="font-semibold text-slate-800">{ticket.kho_xuat?.ten_chinhanh}</div>
            </div>
            <div className={`p-4 rounded-lg border ${isMyImport ? 'border-green-400 bg-green-50' : 'border-slate-200 bg-white'}`}>
              <div className="text-[11px] font-bold text-slate-400 uppercase mb-1">Kho Nhập (Đích) {isMyImport && <span className="text-green-600 font-bold ml-1">(BẠN LÀ BÊN NHẬN)</span>}</div>
                  ...
                  {ticket.trang_thai === 'Chờ duyệt' && isMyExport && (
                    <div className="mt-4 p-3 bg-slate-50 border border-slate-200 rounded">
                      <div className="text-xs font-bold text-slate-700 mb-2">
                        BẠN CẦN CHỌN MÃ SERIAL ĐỂ XUẤT (Đã chọn {selectedSerials[ct.id_chitiet]?.length || 0}/{ct.so_luong})
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {availableSerials[ct.ma_sanpham]?.length > 0 ? (
                          ...
                          <div className="text-sm text-red-500 font-medium">Kho của bạn đã hết sản phẩm này! Vui lòng nhập hàng hoặc từ chối phiếu.</div>
                        )}
                      </div>
                    </div>
                  )}
        ...
        <div className="px-6 py-4 border-t border-slate-200 flex justify-end gap-3 bg-white">
          ...
          {ticket.trang_thai === 'Đang vận chuyển' && isMyExport && (
             <span className="text-xs text-blue-600 font-bold italic flex items-center">🚚 Hàng đang được giao tới kho kia...</span>
          )}
        </div>
      </div>
    </div>
