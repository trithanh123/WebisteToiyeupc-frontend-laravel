...
const VoucherModal = ({ isOpen, onClose, onApply, appliedVoucher, orderTotal }) => {
  ...
  const handleManualApply = () => {
    ...
    const found = vouchers.find(v => (v.ma_voucher || v.Ma_voucher || '').toUpperCase() === inputCode.trim().toUpperCase());
    if (found) {
              ...
              return (
                <div key={v.id_khuyenmai} className={`bg-white border rounded-xl p-4 flex gap-4 items-start ${isApplied ? 'border-blue-500 bg-blue-50/30' : 'border-gray-200'}`}>
                  ...
                  <div className="flex-1">
                    <div className="mb-2">
                      {(v.tenkhuyenmai || v.Tenkhuyenmai) && <h4 className="font-bold text-gray-800 text-[15px] m-0 mb-1.5 leading-snug">{v.tenkhuyenmai || v.Tenkhuyenmai}</h4>}
                      <div className="flex gap-2 items-center flex-wrap">
                        <span className="border border-blue-600 text-blue-700 text-[12px] font-medium px-2 py-0.5 rounded uppercase bg-blue-50/50">{v.ma_voucher || v.Ma_voucher}</span>
                        <span className="font-medium text-gray-800 text-[14px]">
                          Giảm {(v.loai_giamgia || v.Loai_giamgia) === 'Phần trăm' ? `${v.gia_trigiam}%` : formatCurrency(v.gia_trigiam)}
